...
if (isset($cols) && count($cols) > 0) :
    ...
    $base_url = "admin.php?table=" . $selected_table;
    $vind = 0;
    ?>
    <br><a href="<?= $base_url ?>&add_column=1">ADD NEW COLUMN</a>
    <table class="tableinfo" width="100%">
        <tr>
            <?php foreach ($cols as $k => $col) : ?>
                <th>
            <form id="form<?= $vind; ?>" action="admin.php" method="post">
                <?php $helper->get_column_editor($col); ?>
                <?= $col['COLUMN_NAME']; ?> 
                <input type="image" name="submit" src="<?= RESOURCE_PATH; ?>edit.png" border="0" alt="Submit" />
                <!--                <a href="<?= $base_url; ?>" title="Drop Column"><img src="<?= RESOURCE_PATH; ?>delete.png"></a>-->
            </form>
        </th>
    <?php endforeach; ?>
    <th>Actions</th>
